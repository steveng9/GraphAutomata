...
def show_ca_graph(rule_number: int, width: int = 20, timesteps: int = 10,
                  seed: int = 42, show_edges: bool = True, ax=None):
    """
    Draw the PyG graph with nodes placed at their (w, -t) grid coordinates.
    Node colour = cell state (white=0, black=1).
    Edges show the directed causal connections (before undirecting).
    ...
    # Build directed-only edge list for display clarity
    label_map = {rule_number: 0}
    data = grid_to_pyg(grid, rule_number, label_map, undirected=False)
    ...
    for node_id in range(T * W):
        ...
        w = node_id % W
        state = int(grid[t, w])
        G.add_node(node_id, t=t, w=w, state=state)

    ei = data.edge_index.numpy()
    for s, d in zip(ei[0], ei[1]):
        G.add_edge(int(s), int(d))

    # Position: x = column, y = -row (so t=0 is at top)
    ...
    dead = mpatches.Patch(facecolor="white", edgecolor="black", label="state=0")
    alive = mpatches.Patch(facecolor="black", edgecolor="white", label="state=1")
    ax.legend(handles=[dead, alive], loc="upper right", fontsize=8)
    ax.set_title(f"Rule {rule_number} as a graph  ({T*W} nodes, "
                 f"{G.number_of_edges()} directed edges)", fontsize=11)
    ax.set_xlabel("Cell (w) →")
    ax.set_ylabel("← Timestep (t)")
    ax.axis("off")
    return ax
...
def compare_graphs(rules: list, width: int = 15, timesteps: int = 8, seed: int = 42):
    ...
    if len(rules) == 1:
        axes = [axes]
    for ax, r in zip(axes, rules):
        show_ca_graph(r, width=width, timesteps=timesteps, seed=seed, ax=ax)
    fig.suptitle("CA Rules as Graphs  (edges = causal influence)", fontsize=13)
    plt.tight_layout()
    plt.show()
...
if __name__ == "__main__":
    ...
    print("\nGraph view (small crop):")
    compare_graphs([30, 90], width=15, timesteps=8)
